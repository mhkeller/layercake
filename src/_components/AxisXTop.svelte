...
<g class="axis x-axis" class:snapLabels>
	{#if baseline === true}
		<line class="baseline" y1="0" y2="0" x1="0" x2={cake.width} />
	{/if}
	...
	{#each tickVals as tick, i (tick)}
		...
		<g
			class="tick tick-{i}"
			transform="translate({cake.xScale(tick)},{cake.yRange ? Math.min(...cake.yRange) : 0})"
		>
			{#if gridlines === true}
				<line class="gridline" x1="0" x2="0" y1={cake.height} y2="0" />
			{/if}
			{#if tickMarks === true}
				<line
					class="tick-mark"
