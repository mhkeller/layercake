...
<script>
	 ...
	 * @property {Array} [dataset] - The dataset to work off of—defaults to cake.data if left unset. You can pass something custom in here in case you don't want to use the main data or it's in a strange format.
	 */
...
<QuadTree dataset={dataset || cake.data} y="x">
	{#snippet children({ x, y, visible, found, e })}
		...
		{#if visible === true}
			<div style="left:{(x / 100) * cake.width}px;" class="line"></div>
			<div
				class="tooltip"
				style="
	        width:{w}px;
	        display: {visible ? 'block' : 'none'};
	        top:calc({cake.yScale(foundSorted[0].value)}% + {offset}px);
	        left:{Math.min(Math.max(w2, (x / 100) * cake.width), cake.width - w2)}px;"
			>
