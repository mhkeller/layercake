Stacked column chart using D3's [stack](https://github.com/d3/d3-shape#stacks) function. Because this creates a nested data structure, we use Layer Cake's `flatten` function to pass a flat list to the `flatData` prop. See the [client-side example](/example/ColumnStacked) for the same chart with Layer Cake's `stack` helper.
