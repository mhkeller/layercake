<!--
	@component
	Generates an SVG y-axis along the right edge of the chart. If the y scale is a band scale, each tick sits in the middle of its band.
 -->
<script>
...
<g class="axis y-axis">
	{#each tickVals as tick (tick)}
		...
		<g class="tick tick-{tick}" transform="translate({k.xRange ? k.xRange[0] : 0}, {tickValPx})">
			{#if gridlines === true}
				<line class="gridline" x1="0" {x2} y1={halfBand} y2={halfBand}></line>
			{/if}
			{#if tickMarks === true}
				<line
					class="tick-mark"
